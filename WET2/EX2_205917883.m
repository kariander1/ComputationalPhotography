...
ImArray2 = {ImArray2_1 , ImArray2_2 , ImArray2_3,ImArray2_4};
for i_mask = 1:numel(masks_name)
    f_name = join(['ex2_q2_m' num2str(i_mask) '.png']);
    if ~isfile(f_name)
        imwrite(ImArray2{i_mask},f_name);
    end
end

%% Refocus All
mask_ones = ones(16);
if ~exist('focal_stack_refocus','var')
    [~,focal_stack_refocus] = refocusWithMask(s_arr,height,width,lf,mask_ones);
end
[all_focus_I,depth_I] = refocusAll(focal_stack_refocus,s_arr);
imshowpair(all_focus_I,depth_I);

function [focal_stack,focal_stack_arr] = refocusWithMask(s_arr,height,width,lf,mask)
    focal_stack_arr = uint8(zeros(numel(s_arr),height,width,3));
    for  i_s = 1:numel(s_arr)
        s = s_arr(i_s);
        focal_stack_arr(i_s,:,:,:) = refocus(lf,s,mask);
    end
    focal_stack = cat(2,squeeze(focal_stack_arr(1,:,:,:)),squeeze(focal_stack_arr(2,:,:,:)),squeeze(focal_stack_arr(3,:,:,:)));
end
%% Part 2
